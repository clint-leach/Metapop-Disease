highlow.plot <- function(conn){
  ...
  plot(log10(longevity), S.occ[, "+high"], type = "b", lty = 1, ylim = c(0, 1), pch = 19, col = "red", 
       ylab = "S occupancy", xlab = "log(longevity)", main = title("a", adj = 0))
  lines(log10(longevity), S.occ[, "+low"], type = "b", lty = 1, pch = 19, col = "blue")
  
  plot(log10(longevity), I.occ[, "+high"], type = "b", lty = 1, ylim = c(0, 1), pch = 19, col = "red", 
       ylab = "I occupancy", xlab = "log(longevity)", main = title("b", adj = 0))
  lines(log10(longevity), I.occ[, "+low"], type = "b", lty = 1, pch = 19, col = "blue")
  
  occ <- tapply(highlow$I + highlow$S, list(highlow$longevity, highlow$treatment), mean)
  
  plot(log10(longevity), occ[, "+high"], type = "b", lty = 1, ylim = c(0, 1), pch = 19, col = "red",
       ylab = "Total occupancy", xlab = "log(longevity)", main = title("c", adj = 0))
  lines(log10(longevity), occ[, "+low"], type = "b", lty = 1, pch = 19, col = "blue")
